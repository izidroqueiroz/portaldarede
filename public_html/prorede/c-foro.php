...
<table border="0" cellpadding="2" cellspacing="0" style="margin-top:5px;">';
			...
			if (($pol['user_ID'] == $r['user_ID']) AND ($subforo != 'notaria')) {
				...
				$editar = '<span style="float:right;">'.boton('Mover', '/foro/editar/'.$r['ID'], false, 'small').' '.boton(_('Papelera'), accion_url().'a=foro&b=borrar&c=hilo&ID='.$r['ID'], _('¿Quieres enviar a la PAPELERA este HILO y TODOS sus MENSAJES?'), 'small red').'</span>';
			} else { $editar = ''; }
...
<td align="right" width="10%">'.boton(_('Crear Hilo'), (nucleo_acceso($r['acceso_escribir'], $r['acceso_cfg_escribir'])?'/foro/'.$r['url'].'#enviar':false), false, 'large').'</td>
</tr>';
...
LIMIT ".mysql_real_escape_string($r['limite']), $link);
	...
	$txt_title = _('Foro');
	$txt_nav = array('/foro'=>_('Foro'));
	$txt_tab = array('/grupos/'=>_('Foros de grupos').' ('.$foro_oculto_num.')', '/foro/ultima-actividad/'=>_('Última actividad'), '/control/gobierno/foro/'=>_('Configuración foro'), );
