...
$txt .= '<h2>'._('¿Qué hacer').' '._('en').' '.$pol['config']['pais_des'].'?</h2>
...
_('Has votado en todas las votaciones en curso'),
_('Hay votaciones en las que aún no has votado'),
'<a href="/votacion/next" target="_blank" style="font-size:19px;">'._('Ver votaciones').'</a>',
_('Las votaciones (informativas o vinculantes) son el mecanismo democrático más habitual. Duran un tiempo determinado, configuración específica y puede haber varias simultáneas.'));
...
_('Estás ejerciendo ').count(explode(' ', $_SESSION['pol']['cargos'])).' cargos',
_('No estás ejerciendo ningún cargo'),
'<a href="/cargos" target="_blank" style="font-size:19px;">Ver cargos</a>',
_('Los cargos sirven para asignar taréas (con sus responsabilidades y privilegios). Un ejemplo de cargo es por ejemplo los Moderadores o Coordinadores. La asignación de cargos puede ser de 3 tipos: asignado por elecciones periódicas y automáticas, asignado manualmente y asignado al solicitar.'));
...
_('Has repartido todos tus votos de confianza'),
_('Te quedan ').(VOTO_CONFIANZA_MAX-$votos_confianza)._(' votos de confianza por repartir'),
'<a href="/info/censo/confianza" target="_blank" style="font-size:19px;">'._('Votar confianza').'</a>',
_('El voto de confianza es una valoración (+1 -1) que los usuarios se reparten entre sí. No implica que te guste ese usuario, si no que confías en él. Tienes ').VOTO_CONFIANZA_MAX._(' votos de confianza para repartir.'));
...
$txt .= print_hacer($geo,
_('Estás geolocalizado correctamente'),
_('Aún no te has situado en el mapa de ciudadanos'),
'<a href="'.($geo?'/geolocalizacion':'/geolocalizacion/fijar').'" target="_blank" style="font-size:19px;">'._('Ver mapa de ciudadanos').'</a>',
_('El mapa de ciudadanos permite comunicar y conocer a gente cercana geográficamente. De un fácil vistazo puedes encontrar participantes de un lugar concreto y enviarles un mensaje privado. La precisión es de solo 1.111 metros a la redonda, por seguridad.'));
...
_('Tu perfil está bastante completo'),
_('Tienes cosas por rellenar en tu perfil'),
'<a href="/perfil/editar" target="_blank" style="font-size:19px;">'._('Ir a tu perfil').'</a>',
_('Cada usuario tiene su perfil personal, en él se muestra información de actividad, etc. Se recomienda indicar tus redes sociales y escribir una breve nota.'));
...
_('Has votado ').$votos_foro._(' veces en el foro el último mes'),
_('Participa votando más en el foro'),
'<a href="/foro/" target="_blank" style="font-size:19px;">'._('Ir al foro').'</a>',
_('Puedes votar (+1 -1) cualquier hilo y mensaje del foro, así contribuirás al debate con tu opinión. Se recomienda hacer al menos '));
//_('Puedes votar (+1 -1) cualquier hilo y mensaje del foro, así contribuirás al debate con tu opinión. Se recomienda hacer al menos ').$votos_foro_minimo._(' votos al mes (vas ').$votos_foro.').');
...
//THEME
$txt_title = _('¿Qué hacer?');
$txt_nav = array('/hacer'=>_('¿Qué hacer?'));
$txt_menu = 'info';
include('theme.php');
?>