	...
	if (GBrowserIsCompatible()) {
		...
		GEvent.addListener(map, "click", function (overlay,point){
			if (point){
				...
				$("#geo_x").attr("value", point.x, 2);
				$("#geo_y").attr("value", point.y, 2);
			}
		});
...
'._('Latitud').': <input name="y" size="8" type="text" id="geo_y" value="'.$center['y'].'" style="text-align:right;" readonly="readonly" /><br />
'._('Longitud').': <input name="x" size="8" type="text" id="geo_x" value="'.$center['x'].'" style="text-align:right;" readonly="readonly" />
</td>
...
function initialize() {
	...
	for (var i = 0; i < eventos.length; i++) {
		...
		var latlng = new google.maps.LatLng(ev[1], ev[2]);
